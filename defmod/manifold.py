...
from .structuredfield import StructuredField_0, StructuredField_m, CompoundStructuredField
...
class Stiefel(Manifold):
    def __init__(self, dim, nb_pts, gd=None, tan=None, cotan=None):
        assert (gd is None) or (isinstance(gd, tuple) and (len(gd) == 2))
        assert (tan is None) or (isinstance(tan, tuple) and (len(tan) == 2))
        assert (cotan is None) or (isinstance(cotan, tuple) and (len(cotan) == 2))
    ...
    def fill_gd(self, gd, copy=False):
        ...
        if not copy:
            ...
            self.__gd = (gd[0].clone(), gd[1].clone())
    ...
    def fill_tan(self, tan, copy=False):
        ...
        if not copy:
            ...
            self.__tan = (tan[0].clone(), tan[1].clone())
    ...
    def fill_cotan(self, cotan, copy=False):
        ...
        if not copy:
            ...
            self.__cotan = (cotan[0].clone(), cotan[1].clone())
    ...
    def inner_prod_module(self, module):
        ...
        out += sum([torch.tensordot(self.cotan[1].view(-1, self.__dim, self.__dim)[i], man.tan[1].view(-1, self.__dim, self.__dim)[i]) for i in range(self.__mat_shape[0])])

        return out

    def action(self, module) :
        """Applies the vector field generated by the module on the landmark."""
        vx = module.field_generator()(self.__gd[0].view(-1, self.__dim))
        d_vx = module.field_generator()(self.__gd[0].view(-1, self.__dim), k=1)
