...
class Translations(Module):
    ...
    def compute_geodesic_control(self, delta, GeoDesc):
        """ computes geodesic control from \delta \in H^\ast """
        K_q = fun.K_xx(GeoDesc.view(-1, self.dim), self.scale)
        X, _ = torch.gesv(delta.view(-1, self.dim), K_q)
...
class Compound(Module):
    ...
    def __init__(self, Mod_list):
        ...
        self.indiceGeoDesc.extend(np.cumsum([mo.dimGD for mo in Mod_list]))
        self.indiceCont = [0]
        self.indiceCont.extend(np.cumsum([mo.dimCont for mo in Mod_list]))
        self.nb_pts = np.sum(mo.nb_pts for mo in Mod_list)
    ...
    def __call__(self, GeoDesc, Cont, Points) :
        ...
        for i in range(self.Nb_mod):
            list_app.append(self.Mod_list[i](GeoDesc[self.indiceGeoDesc[i]:self.indiceGeoDesc[i+1]], Cont[self.indiceCont[i]:self.indiceCont[i+1]], Points).unsqueeze(0))
    ...
    def action(self, GeoDesc, Mod, GeoDesc_mod, Cont_mod) :
        ...
        list_app = []
        for i in range(self.Nb_mod):
            list_app.append(self.Mod_list[i].action(GeoDesc[self.indiceGeoDesc[i]:self.indiceGeoDesc[i+1]], Mod, GeoDesc_mod, Cont_mod))
